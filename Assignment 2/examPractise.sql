...
\qecho 'Question 1'
\qecho 'Find the pid, name of people who live in Chicago and who know another person who does not have Databases skill.'

with subquery as (select distinct pid1 as pid
                  from knows join personskill on (pid2 = pid) 
                  and skill = 'Databases')
select pid 
from person 
where city = 'Chicago'
...
\qecho 'Find the pid and name of each person who lives in Bloomington and who knows at most one person who has at least 3 job skills.'

with bloomingtonPerson as (select pid from person where city = 'Bloomington'),
     moreThan2Skills as (select pid
                            from personskill 
                            group by(pid)
...
select k.pid1
from knows k, knows k1
where k.pid2 in (select * from moreThan2Skills)
and k1.pid2 in (select * from moreThan2Skills)
and k.pid2 <> k1.pid2
and k.pid1 = k1.pid1)

select * from bloomingtonPerson
intersect 
